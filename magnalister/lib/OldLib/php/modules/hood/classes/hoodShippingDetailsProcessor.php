...
class hoodShippingDetailsProcessor {
	...
	public function renderView($settings = array()) {
		...
		if ($this->args['international']) {
			$services = gethoodInternationalShippingServicesList();
			$locations = gethoodShippingLocationsList();
			...
			$services = gethoodLocalShippingServicesList();
			$locations = array();
			$settings = array_merge(
				array(
					'ShippingService' => '',
		...
		$html = '
			<table id="'.$idkey.'" class="shippingDetails inlinetable nowrap autoWidth"><tbody>
				<tr class="row1">'. (empty($locationSelect) ? '
					...
					' : '
					<td class="paddingRight" colspan="3">'.$serviceSelect.'&nbsp;&nbsp;'.ML_HOOD_LABEL_SHIPPING_COSTS.':&nbsp;'.$shippingCost.'</td>').'
					<td rowspan="2">
						...
						jqml.ajax({
							...
							data: <?php echo json_encode(array_merge(
								...
								array (
									...
									MLHttp::gi()->parseFormFieldName('function') => 'Modul::hoodShippingConfig',
