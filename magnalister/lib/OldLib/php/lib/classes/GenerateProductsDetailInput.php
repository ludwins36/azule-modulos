...
//                new dBug(array($keys,$val,  MLRequest::gi()->data()),'',true);
    ...
    private function getInputValue($curRowC, $isMulticolumn, $subKey = '') {
         ...
         */
        $blAddonBooked =  
            MLShop::gi()->addonBooked('EbayProductIdentifierSync') 
            && MLModul::gi()->getConfig('syncproperties')
        ;
        if (
            (
                (!array_key_exists($this->curInput['key'], $this->data) || $this->data[$this->curInput['key']] == '(matching)')
                || $blAddonBooked
            )
            && in_array($this->curInput['type'], array('text', 'select&text')) 
            && $this->getProduct() !== null
        ) {
            $sValue = MLHelper::gi('Model_Form_Type_Attribute')->fillValue($this->getProduct(), $this->curField['label']);
            if ($sValue !== null && $sValue !== '') {
                $this->curInputReadonly = $blAddonBooked || $sValue == '(matching)';
