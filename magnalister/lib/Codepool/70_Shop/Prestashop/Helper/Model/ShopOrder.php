...
class ML_Prestashop_Helper_Model_ShopOrder {
    ...
    protected function addAddressToOrder($aAddress, $iCustomerId) {
        ...
        if ($iAddressId > 0 && Validate::isLoadedObject($oAddress)) {
            try{
                $oAddress->update();
            } catch (Exception $ex){//normal prestashop needs only one of phone or phone_mobile, but one customer prestashop needed to fill both of them, so we fill both if both are required
                $oAddress->phone_mobile = $oAddress->phone;
                $oAddress->update();
            }
        } else {
            try{
    ...
    protected function addProductToOrder(&$aProduct, OrderState $oOrderStatus) {
        ...
        if (
            ...
            if (
                ...
                $fTaxPercent = ($oProduct->getTax($this->aNewData['AddressSets']) > 0) ? $oProduct->getTax($this->aNewData['AddressSets']) : $fTaxPercent;
            }
        } else {
    ...
    protected function getTax($fTaxPercent) {
        ...
        $tax_id = Db::getInstance()->getValue('select `id_tax` from `' . _DB_PREFIX_ . "tax` where rate = ".(float)$fTaxPercent);
        if (!empty($tax_id)) {
            $oTax = new Tax($tax_id);
        } else {
            $oTax = new Tax();
    ...
    protected function validate($sName, $sType, $iLength = 0, $sRelatedField = '',$blRequired = true) {
        ...
        if (strlen($sValidated) > $iLength) {
            $sValidated = $this->cutValidatedText($sValidated, $iLength, !in_array($sType, array('postcode')));
