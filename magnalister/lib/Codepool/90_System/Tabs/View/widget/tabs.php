    ...
    <div class="magnaTabs2">
        <ul>
            <?php  foreach ($this->getTabs() as $aItem) { ?>
                ...
                ?>
                <li class="<?php echo $aItem['class'] ?> ml-translate-toolbar-wrapper <?php echo $missingTranslationCss?>">
                    <a <?php echo isset($aItem['breadcrumb']) && $aItem['breadcrumb'] !== false ? 'style="pointer-events: none;" class="breadcrumb '.$missingTranslationCss.'"':'class="'.$missingTranslationCss.'"'?>href="<?php echo $aItem['url'] ?>" title="<?php echo str_replace(array('<', '>', '"'), array('&lt;', '&gt;', '&quot;'), $aItem['subtitle'].(empty($aItem['label']) ? '' : '&nbsp;::&nbsp;'.$aItem['label']));?>"<?php echo strpos($aItem['class'], 'inactive') !== false ? ' onclick="return false;"' : '' ?>>
                    ...
                    <?php if ($translationData) { ?>
                        <div class="ml-translate-toolbar">
                            <a href="#" title="Translate label" class="translate-label abutton" <?php echo 'data-ml-translate-modal="#modal-tr-' . str_replace('.', '\\.', $translationData['key']) . '"'; ?>>&nbsp;</a>
                            <div class="ml-modal-translate dialog2" id="modal-tr-<?php echo str_replace('.', '\\.', $translationData['key']) ?>">
