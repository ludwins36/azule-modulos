...
<div id="globalButtonBox"><?php 
    ...
    foreach ($this->getButtons() as $blargh) {
        ...
        } elseif(!MLSetting::gi()->blHideUpdate) {
            $blUpdate = true;
    ...
    (function($) {
        $(document).ready(function(){
            $('#globalButtonBox a.cron').click(function(){
                var cronWarning = JSON.parse($(this).attr('data-warning-text'));
                var cronTitle = JSON.parse($(this).attr('data-warning-title'));
                var hasCron = $(this).hasClass('cron');
                var steps = $(this).data('steps');
                var currentA = $(this);
                $('<div class="ml-modal dialog2" title="' + cronTitle + '">' + cronWarning + '</div>').jDialog({
                    width: '500px',
                    buttons: {
                        Cancel: {
            ...
            $('#ml-show-update-admission').click(function(event) {
                event.preventDefault();
                $('<div class="ml-modal dialog2" title="<?php echo addslashes(MLI18n::gi()->get('ML_MESSAGE_BEFORE_UPDATE_TITLE')) ?>"><?php echo addslashes(MLI18n::gi()->get('ML_MESSAGE_BEFORE_UPDATE_TEXT')) ?></div>').jDialog({
                    width: '500px',
                    buttons: {
                        Cancel: {
                            'text': '<?php echo addslashes(MLI18n::gi()->get('ML_BUTTON_LABEL_ABORT')); ?>',
