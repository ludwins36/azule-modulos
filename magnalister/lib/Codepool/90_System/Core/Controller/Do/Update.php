...
class ML_Core_Controller_Do_Update extends ML_Core_Controller_Abstract {
    ...
    protected $aIgnorePatterns=array(
        ...
        '^Update$',
        '\.svn',
//        'index\.php', //will added automaticly to avoid dirlist
        '^((?!__).*index\.php)$', //will added automaticly in plugin (not shopspecific) to avoid dirlist
    );
...
//                );
    ...
    /**
     * 
     * @param float $fW current value
     * @param float $fG max value
     * @param string $sStep (init, calcSequences, copyFiles, finalizeUpdate, afterUpdate)
     * @return float
     */
    protected function setProgressBarPercent ($fW, $fG, $sStep) {
        $fG = empty($fG) ? 100 : $fG;
        if (array_key_exists($sStep, $this->aProgressBar)) {
            $fWeightingTotal = $fWeightingPrevious = $fWeightingCurrent = 0;
    ...
    public function progressBar ($sCurrentStep, $mFinalize = null) {
        MLController::gi('widget_progressbar')->setId('updatePlugin')->setContent('');
        $blStepFound = false;
        $sNextStep = '';
        ...
        if ($mFinalize === true && $sNextStep == '') {
            ...
            if (MLSetting::gi()->get('blDebug')) {// remove autom. redirect, show sequenzes
                ...
                MLController::gi('widget_progressbar')->addLog('<a style="color:gray;" onclick="jqml(this).html(\'\');return true;" class="global-ajax" data-ml-global-ajax=\'{"triggerAfterSuccess":"currentUrl"}\' href="'.$this->getCurrentUrl(array('method'=>$sNextStep)).'">Click here for next step ('.$sNextStep.').</a>');
            } else {
            ...
            MLController::gi('widget_progressbar')
                ...
                ->addLog('<span style="color:red">A error has occurred<br />&nbsp;&nbsp;&nbsp;&nbsp;Message: '.$mFinalize->getMessage().'<br />&nbsp;&nbsp;&nbsp;&nbsp;Code: '.$mFinalize->getCode().'</span>')
            ;
            MLController::gi('widget_progressbar')->render();
        } else {
            if (MLSetting::gi()->get('blDebug')) {
                MLController::gi('widget_progressbar')->addLog('# currentStep: '.$sCurrentStep);
            }
    ...
    public function callAjaxAddIndexPhp () {
        ...
        try {
            MLController::gi('widget_progressbar')->addLog('# '.$this->addIndexPhp()." index.php files created.\n");
    ...
    public function callAjaxCalcSequences () {
        ...
        try {
            ...
            if (MLSetting::gi()->get('blDebug')) {// remove autom. redirect, show sequenzes
                ...
                MLController::gi('widget_progressbar')->addLog('# Sequences are calculated, see debug-bar for more info.');
            }
