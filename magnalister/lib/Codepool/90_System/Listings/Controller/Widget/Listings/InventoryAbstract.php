...
class ML_Listings_Controller_Widget_Listings_InventoryAbstract extends ML_Listings_Controller_Widget_Listings_ListingAbstract {
    ...
    public function prepareData() {
        ...
        if (($result !== false) && !empty($result['DATA'])) {
            ...
            foreach ($this->aData as &$item) {
                ...
                $item['ShopPrice'] = $oProduct->exists() ? $oProduct->getShopPrice() : '';
    ...
    protected function getFields() {
        return array(
            ...
            'Price' => array(
                'Label' => 'Shop-'.MLI18n::gi()->ML_GENERIC_PRICE . ' / ' . MLModul::gi()->getMarketPlaceName(false),
                'Sorter' => 'price',
                'Getter' => 'getItemPrice',
                'Field' => null
            ),
    ...
    protected function getItemPrice($item) {
        ...
        $renderedShopPrice = !empty($item['ShopPrice']) ? MLPrice::factory()->format($item['ShopPrice'], $item['Currency']) : '&mdash;';
        return '<td>' . $renderedShopPrice . ' / ' . ( !empty($item['Price']) ? MLPrice::factory()->format($item['Price'], $item['Currency']) : '&mdash;') . '</td>';
    }
