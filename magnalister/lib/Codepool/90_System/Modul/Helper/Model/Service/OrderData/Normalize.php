...
class ML_Modul_Helper_Model_Service_OrderData_Normalize {
    ...
    protected function getPaymentCode($aTotal, $sPaymentMethodConfigKey = 'orderimport.paymentmethod') {
        $sPaymentMethodConfig = MLModul::gi()->getConfig($sPaymentMethodConfigKey);
        $sPaymentMethod = '';
        if ($sPaymentMethodConfig === 'textfield') {
            $sPaymentMethod = MLModul::gi()->getConfig($sPaymentMethodConfigKey.'.name');
        ...
        return empty($sPaymentMethod) ? 'magnalister - '.MLModul::gi()->getMarketPlaceName(false) : $sPaymentMethod;
    }
