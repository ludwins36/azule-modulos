...
//    protected $blValidateAuthKeys = true;
    ...
    public function __construct() {
        foreach (array('SyncOrderStatus', 'SyncInventory', 'ImportOrders') as $sSync) {
            MLSetting::gi()->set('s'.$sSync.'Url', MLHttp::gi()->getFrontendDoUrl(array('do' => $sSync, 'auth' => md5(MLShop::gi()->getShopId().trim(MLDatabase::factory('config')->set('mpid', 0)->set('mkey', 'general.passphrase')->get('value'))))), true);
        }
        parent::__construct();
    }
            ...
            foreach ($this->aRequestFields as $sName => $mValue) {
                if ($sName == 'tabident') {
                    ...
                    if (array_key_exists($sName, $aAuthKeyDefinition)) {
                        MLModul::gi()->resetIsAuthedErrorCaches();
                        $mValue = trim($mValue);
