...
if ($this instanceof ML_Productlist_Controller_Widget_ProductList_Abstract) {
    try {
        ...
        $sCValue = (!empty($this->getPreparedInfo($oProduct, 'currencyid'))) ? current($this->getPreparedInfo($oProduct, 'currencyid')) : ''; // determine if the Item is prepared at all
        $sEValue = (!empty($this->getPreparedInfo($oProduct, 'epid'))) ? current($this->getPreparedInfo($oProduct, 'epid')) : '';
