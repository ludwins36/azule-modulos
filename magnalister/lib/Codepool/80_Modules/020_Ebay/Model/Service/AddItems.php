...
class ML_Ebay_Model_Service_AddItems extends ML_Modul_Model_Service_AddItems_Abstract {
    ...
    protected function getProductArray() {
        ...
        try {
            ...
            //            MLMessage::gi()->addDebug('aOut',$this->aOut);
            //            die;
        } catch (Exception $oEx) {
            echo $oEx->getMessage();
        }
        //$iId = $this->oCurrentProduct->get('id');
        //$this->aOut[$iId]['rawDescription'] = stringToUTF8(html_entity_decode(fixHTMLUTF8Entities(MLProduct::factory()->set('id', $iId)->getDescription())));
    ...
    protected function setMasterVariation() {
        ...
        foreach ($this->oList->getList() as $oMaster) {
            ...
            if (count($aVariants) == 1 && current($aVariants)->getVariatonData() == array()) {
                ...
                foreach (array('Title', 'Description', 'PictureURL', 'GalleryType', 'Subtitle', 'StartPrice') as $sField) {
    ...
    protected function replaceVariationPictures($mValue, $aData) {
        ...
        if (is_array($mValue) && array_key_exists($sVariationDimension, $mValue)) {
            ...
            foreach ($mValue[$sVariationDimension] as $sKey => $aImages) {
                $sBasePricedKey = array_key_exists($sKey, $this->aVariationTranslationBasePrice) ? $this->aVariationTranslationBasePrice[$sKey] : $sKey;
                if (!empty($aImages)) {
