...
class ML_PriceMinister_Controller_PriceMinister_Prepare_Match_Manual extends ML_Form_Controller_Widget_Form_PrepareWithVariationMatchingAbstract
{
    ...
    protected function triggerBeforeFinalizePrepareAction()
    {
        ...
        foreach ($_POST['match'] as $key => $itemID){
            if ($itemID !== 'false'){
                $prepareItems['[' . $key . ']']->set('ItemTitle', $_POST['matching'][$key]['title']);
                ...
                if ($this->oPrepareHelper->bIsSinglePrepare){
                    ...
                    $shopVariation = $this->getMatchedVariations($_POST['matching'][$key]['category_id']);
                    $prepareItems['[' . $key . ']']->set('TopPrimaryCategory', $_POST['matching'][$key]['category_id']);
                    $prepareItems['[' . $key . ']']->set('PrimaryCategory', $_POST['matching'][$key]['category_id']);

                    if (empty($shopVariation)){
                        $this->missingCategories[$_POST['matching'][$key]['category_name']] = $_POST['matching'][$key]['category_name'];
                        $prepareItems['[' . $key . ']']->set('verified', 'ERROR');
                    } else {
    ...
    protected function callAjaxItemSearchByKW()
    {
        $aProduct = array(
            'Id' => $_POST['productID'],
            'Results' => $this->oPrepareHelper->searchOnPriceMinister($_POST['search'], 'KW')
        );
    ...
    protected function callAjaxItemSearchByEAN()
    {
        $aProduct = array(
            'Id' => $_POST['productID'],
            'Results' => $this->oPrepareHelper->searchOnPriceMinister($_POST['ean'], 'EAN')
        );
    ...
    protected function callAjaxItemSearchByCategory()
    {
        $aProduct = array(
            'Id' => $_POST['productID'],
            'Results' => $this->oPrepareHelper->searchOnPriceMinister($_POST['cat'], 'Category')
        );
    ...
    protected function callAjaxItemSearchByPMProductId()
    {
        $aProduct = array(
            'Id' => $_POST['productID'],
            'Results' => $this->oPrepareHelper->searchOnPriceMinister($_POST['pmpid'], 'productids')
        );
    ...
    {
        ob_start();
        $this->includeView('widget_form_type_productvariations', array('mParentValue' => $_POST['category_id']));
