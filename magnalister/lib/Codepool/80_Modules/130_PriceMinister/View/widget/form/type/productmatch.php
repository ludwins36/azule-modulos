    ...
    <table class="matching">
        ...
        <tr>
            <th colspan="5">
                ...
                <input type="hidden" name="matching[<?= $aProduct['Id'] ?>][title]"
                       id="match_title_<?= $aProduct['Id'] ?>"/>
                <input type="hidden" name="matching[<?= $aProduct['Id'] ?>][category_id]"
                       id="match_category_id_<?= $aProduct['Id'] ?>"/>
                <input type="hidden" name="matching[<?= $aProduct['Id'] ?>][category_name]"
                       id="match_category_name_<?= $aProduct['Id'] ?>"/>
                <input type="hidden" name="matching[<?= $aProduct['Id'] ?>][product_id]"
                ...
                <button type="button" value="0" name="ml[action][prepareaction]"
                ...
                <div id="productDetails_<?= $aProduct['Id'] ?>" class="productDescBtn"
                     title="<?= ML_LABEL_DETAILS ?>"><?= ML_LABEL_DETAILS ?></div>
            </th>
        </tr>
        </tbody>
            ...
            (function () {
                ...
                jqml('#newPMpIdGo_<?= $aProduct['Id'] ?>').click(function() {
                    ...
                    if(jqml.trim(newSearch) != '') {
                        ...
                        jqml.ajax({
                            ...
                            url: '<?php echo $this->getCurrentUrl() ?>',
                            data: appendFormFields({
                                'ml[method]': 'ItemSearchByPMProductId',
                                'ml[ajax]': true,
                                'productID': <?= $aProduct['Id'] ?>,
                                'pmpid': newSearch
                            }),
                ...
                jqml('#newCatGo_<?= $aProduct['Id'] ?>').click(function() {
                    ...
                    if(jqml.trim(newSearch) != '') {
                        ...
                        jqml.ajax({
                            ...
                            url: '<?php echo $this->getCurrentUrl() ?>',
                            data: appendFormFields({
                                'ml[method]': 'ItemSearchByCategory',
                                'ml[ajax]': true,
                                'productID': <?= $aProduct['Id'] ?>,
                                'cat': newSearch
                            }),
                ...
                jqml('#priceminister_prepare_match_manual_fieldset_manualmatching').on('change', 'input:radio', function() {
                    ...
                    jqml.ajax({
                        ...
                        url: '<?php echo $this->getCurrentUrl() ?>',
                        data: appendFormFields({
                            'ml[method]': 'AdvertAttrForCategory',
                            'ml[ajax]': true,
                ...
                jqml(document).on('click', '#priceminister_prepare_match_manual_form_field_prepareaction', function() {
                    ...
                    jqml.ajax({
                        ...
                        url: '<?php echo $this->getCurrentUrl() ?>',
                        data: appendFormFields({
                            'ml[method]': 'PrepareAdvertAttribute',
                            'ml[ajax]': true,
                            'productID': selectedProductId,
                            'formdata': formData,
                            'category_id': categoryId
