...
MLI18n::gi()->{'ricardo_text_quantity'} = 'ricardo.ch generally don’t allow stock-increase for running offers.<br>
...
ricardo.ch may charge slotting fees for that!<br>
...
MLI18n::gi()->{'ricardo_text_price'} = 'ricardo.ch generally don’t allow price-increase for running offers.<br>
...
Please confirm that you read and understand this information by clicking “accept” or undo the action without activating that function.
';
MLI18n::gi()->{'ricardo_config_error_price_signal'} = 'Prices on ricardo.ch must be specified in Swiss franc. Please also adapt the price (last decimal number) so that it is 0 (eg. 12.40) or 5 (eg. 12.45). The lowest amount possible is 5 Rappen (0.00 CHF). Click the info-icon "number after decimal point" for more details.';
...
MLI18n::gi()->{'ricardo_config_account__field__token__label'} = 'ricardo.ch Token';
MLI18n::gi()->{'ricardo_config_account__field__token__help'} = 'To apply for a new ricardo.ch token, please click the button.<br>
If this does not open ricardo.ch in a new window, please deactivate your popup blocker.<br><br>
The token is needed to allow you to access ricardo.ch via the magnalister interface.<br>
Please follow the steps in the ricardo.ch window to apply for a token and connect your online Shop with ricardo.ch via magnalister.';
...
MLI18n::gi()->{'ricardo_config_prepare__field__priceforauction__label'} = 'startin price for auction (CHF)';
...
MLI18n::gi()->{'ricardo_config_price__field__mwst__help'} = 'VAT amount that is considered in the article-upload to ricardo.ch. Standard web-shop-VAT will be taken if you leave this field open.';
...
<br><br>
                    <b>Note:<br>ricardo.ch has an availablity limit. Please make sure that the stock of each article you are offering on the ricardo.ch marketplace does not exceed 999 units</b><br> The settings in \'Configuration\' ,&rarr; ‘Article upload:preset’  &rarr; ‘Stock quantity’ will the taken into account.';
MLI18n::gi()->{'ricardo_config_sync__field__stocksync.frommarketplace__label'} = 'Stocke change Ricardo';
...
MLI18n::gi()->{'ricardo_config_orderimport__field__orderstatus.sync__help'} = '<dl>
            <dt>Automatic Synchronization via CronJob (recommended)</dt>
                    <dd>Current Ricardo stock will be synchronized with shop stock every 4 hours, beginning at 0.00am (with ***, depending on configuration).<br>Values will be transferred from the database, including the changes that occur through an ERP or similar.<br><br>
Manual comparison can be activated by clicking the corresponding button in the magnalister header (left of the shopping cart).<br><br>
Additionally, you can activate the stock comparison through CronJon (flat tariff*** - maximum every 4 hours) with the link:<br>
            <i>{#setting:sSyncInventoryUrl#}</i><br>
