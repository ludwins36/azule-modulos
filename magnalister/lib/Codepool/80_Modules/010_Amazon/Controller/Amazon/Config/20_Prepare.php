    ...
    private function validateB2BDiscountTier(&$aField, $prevFieldId, $tierNumber, $quantityField = false)
    {
        ...
        if ($this->getField('b2bactive', 'value') == 'true' && !empty($tierType)) {
            if (!$quantityField) {
                ...
                // Check it for discount field only to prevent circular reference and because quantity field
                // is already rendered.
                ...
                if (($value == 0 && $quantityValue != 0) || ($value != 0 && $quantityValue == 0)) {
                    MLMessage::gi()->addError(
                        MLI18n::gi()->get('amazon_config_tier_error', array('TierNumber' => $tierNumber)));
                    $aField['cssclasses'][] = 'ml-error';
                }
            }

            if ($value < 0 || ($value > 0 && ($previousValue == 0 || $value <= $previousValue))) {
                MLMessage::gi()->addError(
                    MLI18n::gi()->get('amazon_config_tier_error', array('TierNumber' => $tierNumber)));
                $aField['cssclasses'][] = 'ml-error';
            }
