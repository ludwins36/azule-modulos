...
class Ws_WishList extends ObjectModel
{
    ...
    public static function importData()
    {
        ...
        if (!empty($countReviews)) {
            $sQuery = 'SELECT * FROM ' . _DB_PREFIX_ . 'wishlist';
    
            $mReturn = Db::getInstance()->ExecuteS($sQuery);
    
            $err = '';
            foreach ($mReturn as $str_comment) {
                $iQuery = 'INSERT INTO `' . _DB_PREFIX_ . 'ws_wishlist` (`id_wishlist`, `id_customer`, `token`, `name`, `counter`, `id_shop`, `id_shop_group`, `date_add`, `date_upd`, `default`)
                    VALUES ("'.(int)$str_comment["id_wishlist"].'", "'.(int)$str_comment["id_customer"].'", "'.pSQL($str_comment["token"]).'", "'.pSQL($str_comment["name"]).'", "'.pSQL($str_comment["counter"]).'", "'.(int)$str_comment["id_shop"].'", "'.(int)$str_comment["id_shop_group"].'", "'.pSQL($str_comment["date_add"]).'", "'.pSQL($str_comment["date_upd"]).'", "'.(int)$str_comment["default"].'")';
                Db::getInstance()->execute($iQuery);
                $err .= Db::getInstance()->getMsgError();
            }
            
            $sQuery2 = 'SELECT * FROM ' . _DB_PREFIX_ . 'wishlist_product';
            $mReturn2 = Db::getInstance()->ExecuteS($sQuery2);

            foreach ($mReturn2 as $str_comment) {
                $iQuery = 'INSERT INTO `' . _DB_PREFIX_ . 'ws_wishlist_product` (`id_wishlist_product`, `id_wishlist`, `id_product`, `id_product_attribute`, `quantity`, `priority`)
                    VALUES (NULL, "'.(int)$str_comment["id_wishlist"].'", "'.(int)$str_comment["id_product"].'", "'.(int)$str_comment["id_product_attribute"].'", "'.pSQL($str_comment["quantity"]).'", "'.pSQL($str_comment["priority"]).'")';
                Db::getInstance()->execute($iQuery);
                $err .= Db::getInstance()->getMsgError();
            }
        }
    ...
    * Get Customers having a wishlist
     ...
     * @return array Results
     */
    public static function getByIdCustomer($id_customer, $default = false)
    {
        ...
        if (!Cache::isStored($cache_id)) {
            ...
            FROM `'._DB_PREFIX_.'ws_wishlist` w
            WHERE `id_customer` = '.(int)($id_customer).$def_sql.'
            '.$shop_restriction.'
            orDER BY w.`name` ASC'
            );
            Cache::store($cache_id, $result);
        ...
        if (isset($freshwish) and $freshwish != false) {
            foreach ($freshwish as $prodcustomer) {
                Db::getInstance()->execute(
                    ...
                    (
                        SELECT `quantity` FROM `'._DB_PREFIX_.'ws_wishlist_product_cart`
    ...
    public static function getProductByIdCustomer($id_wishlist, $id_customer, $id_lang, $id_product = null, $quantity = false)
    {
        ...
        $t_products = Db::getInstance()->executeS(
            'SELECT wp.`id_product`, wp.`quantity` as wl_quantity, 
                p.*,
                p.`quantity` AS product_quantity, 
                pl.`name`, pl.link_rewrite,
                wp.`id_product_attribute`, wp.`priority`, 
        ...
        for ($i = 0; $i < sizeof($products); ++$i) {
            if (Configuration::get('ADVANSEDWISHLIST_TAX')) {
    ...
    public static function getLightProductByIdCustomer($id_wishlist, $id_customer, $id_lang, $id_product = null, $quantity = false)
    {
        if (!Validate::isUnsignedId($id_customer) or
        !Validate::isUnsignedId($id_lang) or
        !Validate::isUnsignedId($id_wishlist)) {
     ...
     * @return array Results
     */
    public static function getInfosByIdCustomer($id_customer, $default = false)
    {
        if (Shop::getContextShopID()) {
            $shop_restriction = 'AND id_shop = '.(int)Shop::getContextShopID();
        } elseif (Shop::getContextShopGroupID()) {
        ...
        WHERE w.`id_customer` = '.(int)($id_customer).$def_sql.'
        '.$shop_restriction.'
    ...
    public static function getIdByIdCustomer($id_customer)
    {
        if (Shop::getContextShopID()) {
            $shop_restriction = 'AND id_shop = '.(int)Shop::getContextShopID();
        } elseif (Shop::getContextShopGroupID()) {
        ...
        return (Db::getInstance(_PS_USE_SQL_SLAVE_)->executeS(
          ...
          FROM  `'._DB_PREFIX_.'ws_wishlist` w 
        WHERE w.`id_customer` = '.(int)($id_customer).' AND w.`default` = 1 
        '.$shop_restriction.'
        GROUP BY w.`id_wishlist`
        orDER BY w.`name` ASC'
        )
    ...
    public static function issetProduct($id_wishlist, $id_product, $id_product_attribute = false)
    {
        return (Db::getInstance(_PS_USE_SQL_SLAVE_)->getValue(
            ...
            ($id_product_attribute != false ? ' AND wp.`id_product_attribute` = '.(int)($id_product_attribute) : '')
        )
        );
    }
    ...
    */
    ...
    public static function getTopProductsList()
    {
        $product_list = Db::getInstance()->executeS('
                SELECT wp.`id_product`, 
                (SELECT i.`id_image` FROM '._DB_PREFIX_.'image i WHERE i.`id_product` = wp.`id_product` ORDER BY i.`cover` DESC LIMIT 0,1) as id_image, 
                pl.`link_rewrite`, pl.`name`, SUM(wp.`quantity`) as qty_sum 
                FROM `'._DB_PREFIX_.'ws_wishlist_product` as wp 
                LEFT JOIN `'._DB_PREFIX_.'product_lang` pl ON (wp.`id_product` = pl.`id_product` AND pl.`id_lang` = '.(int)Context::getContext()->language->id.')
                WHERE 1 GROUP BY wp.`id_product`');
        ...
        for ($i = 0; $i < count($product_list); $i++) {
            $product_list[$i]['image'] = Context::getContext()->link->getImageLink(Tools::strtolower($product_list[$i]['link_rewrite']), $product_list[$i]['id_product'].'-'.$product_list[$i]['id_image'], $f_small);
        }
        
        $qty_sum = array_column($product_list, 'qty_sum');
        array_multisort($qty_sum, SORT_DESC, $product_list);
    ...
    */
    public function setDefault()
    {
        ...
        Context::getContext()->cookie->id_wishlist = (int)$this->id;
